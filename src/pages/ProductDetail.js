...
const ProductDetail = () => {
  ...
  const handleAddToCart = (shouldOpenDrawer = true) => {
    ...
    const variant = {
      ...
      stock: selectedSizeObj?.stock || 1
    };

    addToCart(product, variant, quantity, shouldOpenDrawer);
    toast.success(isRTL ? `تمت إضافة "${product.name}" إلى السلة` : `Added "${product.name}" to cart!`);
    analyticsAPI.trackEvent({ type: 'add_to_cart', productId: product._id, page: 'product' }).catch(() => {});
  };
      ...
      <div className="container">
        <div className="product-layout">
          ...
          <div className="product-details">
            ...
            {/* Quantity Stepper (Shown for Card Checkout or general purchase) */}
            <div className="product-qty-row" style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
              <span style={{ fontSize: '0.9rem', fontWeight: '600', color: '#1A1612' }}>{t('quantity') || 'Quantity'}:</span>
              <div className="quantity-stepper" style={{ display: 'flex', alignItems: 'center', background: '#FFFFFF', border: '1px solid #D8CFBF', borderRadius: '4px' }}>
                <button 
                  type="button" 
                  onClick={() => setQuantity(prev => Math.max(1, prev - 1))}
                  disabled={quantity <= 1}
                  style={{ padding: '6px 12px', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}
                >
                  -
                </button>
                ...
                <button 
                  type="button" 
                  onClick={() => setQuantity(prev => (selectedSizeObj?.stock ? Math.min(selectedSizeObj.stock, prev + 1) : prev + 1))}
                  disabled={selectedSizeObj?.stock ? quantity >= selectedSizeObj.stock : false}
                  style={{ padding: '6px 12px', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}
                >
                  +
                </button>
              ...
              {selectedSizeObj?.stock > 0 && (
                <span style={{ fontSize: '0.8rem', color: '#7A6C58' }}>
                  ({selectedSizeObj.stock} {t('leftInStock') || 'left'})
                </span>
              )}
            </div>
            ...
            {isCardModel ? (
              <div className="card-model-actions" style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }}>
                <button
                  className="btn btn-primary add-to-cart-btn"
                  onClick={() => handleAddToCart(true)}
                  disabled={isProductSoldOut || isSizeOutOfStock}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '1rem', fontSize: '1rem' }}
                >
                  <FiShoppingBag />
                  {isProductSoldOut ? t('soldOut') : isSizeOutOfStock ? t('sizeOutOfStock') : (t('addToCart') || 'Add to Cart')}
                </button>
                <button
                  className="btn btn-outline buy-now-btn"
                  onClick={handleBuyNow}
                  disabled={isProductSoldOut || isSizeOutOfStock}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '0.9rem', fontSize: '0.95rem' }}
                >
                  <FiCreditCard />
                  {isRTL ? 'شراء الآن والدفع' : 'Buy Now & Checkout'}
                </button>
              </div>
            ) : (
              <button
